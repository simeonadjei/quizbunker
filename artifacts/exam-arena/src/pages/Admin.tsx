      ...
      <div className="max-w-6xl mx-auto space-y-12">
        <header className="border-b border-red-500/30 pb-4 mb-8 flex items-start justify-between gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-gray-200 hover:bg-red-950 border border-gray-300 hover:border-red-500/60 text-gray-500 hover:text-red-400 text-sm font-bold rounded transition-colors shrink-0"
            title="Lock screen"
          >
...
function PendingPaymentsPanel() {
  ...
  return (
    <div>
      ...
      {pending.length === 0 ? (
        <div className="rounded border border-gray-200 px-4 py-8 text-center text-gray-400 text-sm">
          No pending payments.
        </div>
      ) : (
          ...
          {pending.map(p => (
            <div key={p.id} className="rounded border border-yellow-500/20 bg-white p-4">
              <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs mb-3">
                ...
                <span className="text-gray-500"><span className="text-gray-400">AMOUNT:</span> GHS {((p.amount ?? 0) / 100).toFixed(2)}</span>
                <span className="text-gray-500"><span className="text-gray-400">DATE:</span> {format(new Date(p.createdAt), 'MM-dd HH:mm')}</span>
                {p.status === 'mismatch' && (
                  <span className="text-red-400 font-bold">MISMATCH — user re-notified</span>
                )}
              </div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-gray-400 text-xs shrink-0">USER'S TX ID:</span>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-gray-50 border border-gray-300 text-gray-700 font-mono text-sm rounded px-3 py-2 placeholder:text-gray-400 focus:outline-none focus:border-yellow-500"
                />
                <button
...
function ManualSubscribePanel() {
  ...
  const inputCls = "w-full bg-gray-50 border border-gray-300 text-gray-700 text-sm rounded px-3 py-2 placeholder:text-gray-400 focus:outline-none focus:border-green-500";
  ...
  return (
    <div className="bg-white border border-gray-200 p-6">
      ...
      {/* Mode tabs */}
      <div className="flex gap-1 mb-5 bg-gray-50 border border-gray-200 rounded p-1 w-fit">
        <button
          onClick={() => { setMode('existing'); setResult(null); }}
          className={`px-4 py-1.5 text-xs font-bold rounded transition-colors ${mode === 'existing' ? 'bg-green-800 text-white' : 'text-gray-400 hover:text-gray-700'}`}
        >
          Existing User
        </button>
        <button
          onClick={() => { setMode('new'); setResult(null); }}
          className={`px-4 py-1.5 text-xs font-bold rounded transition-colors ${mode === 'new' ? 'bg-cyan-800 text-white' : 'text-gray-400 hover:text-gray-700'}`}
        >
          New User
        </button>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-4">
        ...
        {mode === 'existing' && (
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="text-gray-500 text-sm">Generate &amp; email new password</span>
          </div>
        )}
      ...
      {result && (
        <div className="mt-4 p-4 bg-gray-50 border border-green-500/30 text-green-400 text-sm space-y-1 rounded">
          ...
          {result.generatedPassword && (
            <div className="mt-2 text-cyan-400">
              ...
              <span className="text-gray-400 text-xs ml-2">(emailed to user)</span>
            </div>
          )}
        </div>
      )}
    ...
    <div>
      ...
      <div className="overflow-x-auto rounded border border-gray-200">
        <table className="w-full text-xs">
          <thead className="bg-white text-gray-400 text-left uppercase tracking-wider">
            <tr>
          ...
          <tbody className="divide-y divide-gray-200">
            {logs.length === 0 && !isFetching && (
              <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400">No activity yet</td></tr>
            )}
            {logs.map(log => (
              <tr key={log.id} className="hover:bg-gray-50">
                ...
                <td className={`px-4 py-2.5 font-mono font-bold whitespace-nowrap ${TYPE_COLORS[log.type] ?? 'text-gray-700'}`}>
                  {log.type}
                </td>
                ...
                <td className="px-4 py-2.5 text-gray-400 font-mono text-[10px] max-w-xs truncate">
                  {log.metadata ?? ''}
                </td>
              </tr>
            ))}
        ...
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-gray-200">
            {payments.length === 0 && !isFetching && (
              <tr><td colSpan={7} className="px-4 py-6 text-center text-gray-400">No payments yet</td></tr>
            )}
            {payments.map(p => (
              <tr key={p.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2.5 text-gray-700">
                  <div>{p.userName ?? '—'}</div>
                  <div className="text-gray-400">{p.userEmail ?? ''}</div>
                </td>
                <td className="px-4 py-2.5 text-gray-700 uppercase">{p.plan}</td>
                <td className="px-4 py-2.5 text-gray-700">GHS {(p.amount / 100).toFixed(2)}</td>
                <td className="px-4 py-2.5">
                  <span className={`px-2 py-0.5 rounded text-[10px] ${STATUS_COLORS[p.status] ?? 'text-gray-400'}`}>
...
function AdminLogin({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <form onSubmit={handleSubmit} className="border border-red-500/50 p-10 bg-white w-full max-w-2xl rounded font-mono shadow-[0_0_60px_-5px_rgba(239,68,68,0.4)]">
        ...
        <Input
          ...
          className="bg-gray-50 border-red-500/30 text-red-500 focus-visible:ring-red-500 rounded-none mb-4 h-14 text-lg"
        ...
        <Input
          ...
          className="bg-gray-50 border-red-500/30 text-red-500 focus-visible:ring-red-500 rounded-none mb-6 h-14 text-lg"
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
      {[
        ...
        <div key={s.label} className="bg-white border border-gray-200 p-4">
          ...
          <div className={`text-2xl font-bold ${s.color || 'text-gray-900'}`}>{s.val}</div>
        </div>
      ))}
    </div>
  );
    ...
    <div className="bg-white border border-gray-200 p-6 flex flex-col">
      ...
      <div className="space-y-4">
        <div>
          ...
          {files.length > 1 && (
            <div className="mt-2 space-y-1 max-h-40 overflow-y-auto">
              {files.map((f, i) => (
                <div key={i} className="flex items-center justify-between text-xs text-gray-500 bg-gray-50 border border-gray-200 px-3 py-1.5">
          ...
          {textPreview && (
            <div className="mt-2">
              <div className="text-xs text-gray-400 mb-1">EXTRACTED TEXT PREVIEW (first 400 chars):</div>
              <pre className="text-xs text-gray-500 bg-gray-50 border border-gray-200 p-2 overflow-x-auto whitespace-pre-wrap break-words max-h-32">{textPreview}</pre>
              <div className="text-xs text-gray-400 mt-1">Expected format: <span className="text-gray-500">Year 1 Science / WEEK 1: TOPIC / 1. Question / A. Option / Answer: A</span></div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-gray-400">YEAR OVERRIDE (OPTIONAL)</label>
            <Input value={year} onChange={(e) => setYear(e.target.value)} className="bg-gray-50 border-gray-300 mt-1" placeholder="e.g. 2023" disabled={isUploading} />
          </div>
          <div>
            <label className="text-xs text-gray-400">SUBJECT OVERRIDE (OPTIONAL)</label>
            <Input value={subject} onChange={(e) => setSubject(e.target.value)} className="bg-gray-50 border-gray-300 mt-1" placeholder="e.g. MATH" disabled={isUploading} />
          </div>
        </div>
        ...
        {summary && (
          <div className="mt-4 p-4 bg-gray-50 border border-green-500/30 text-sm space-y-1">
            ...
            <div className="text-gray-500">SKIPPED: {summary.skipped}</div>
      ...
      <div className="mt-6">
        ...
        <div className="bg-gray-50 border border-gray-200 divide-y divide-gray-100">
          {groupList.length === 0 ? (
            <div className="px-3 py-4 text-xs text-gray-400 text-center">No questions uploaded yet</div>
          ) : groupList.map(g => {
            const key = `${g.year}|${g.subject}`;
            const isDeleting = deletingGroup === key;
            return (
              <div key={key} className="flex items-center justify-between px-3 py-2.5 hover:bg-gray-50">
                <div>
                  <span className="text-sm font-bold text-gray-800">{g.year}</span>
  ...
  return (
    <div className="bg-white border border-gray-200 p-6 flex flex-col h-full">
      ...
      <div className="space-y-4 mb-8">
        <div>
          <label className="text-xs text-gray-400">AUDIO FILES (select multiple)</label>
          <Input
            type="file"
            accept="audio/*"
            multiple
            onChange={(e) => setFiles(Array.from(e.target.files || []))}
            className="bg-gray-50 border-gray-300 mt-1 p-1 text-sm"
            disabled={isUploading}
          />
          {files.length > 0 && (
            <div className="mt-2 space-y-1">
      ...
      <div className="flex-1 overflow-auto bg-gray-50 border border-gray-200 p-2">
        {songs.length === 0 && (
          <p className="text-xs text-gray-400 text-center py-4">No songs yet</p>
        )}
        {songs.map(song => (
          <div key={song.id} className="flex items-center justify-between p-2 border-b border-gray-100 hover:bg-gray-50">
    ...
    <div className="bg-white border border-gray-200 p-6">
      ...
      <div className="flex gap-2 items-center">
        <input
          ...
          className="flex-1 bg-gray-50 border border-gray-300 text-gray-700 text-sm rounded px-3 py-2 placeholder:text-gray-400 focus:outline-none focus:border-gray-400"
        />
        <button
          onClick={handleTest}
...
function ReferralEarningsPanel() {
  ...
  return (
    <div className="bg-white border border-yellow-600/40 p-6">
      <div className="flex items-center justify-between mb-4">
      ...
      {!isLoading && rows.length === 0 && (
        <p className="text-gray-400 text-sm italic py-4">No referral earnings yet. Earnings appear here after a referred user subscribes.</p>
      )}
      ...
      {!isLoading && rows.length > 0 && (
        <div className="space-y-6">
          {rows.map((row) => {
            ...
            return (
              <div key={row.userId} className="border border-gray-200 rounded p-4 space-y-3">
                {/* User header */}
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-bold text-white">{row.userName}</p>
                    <p className="text-gray-400 text-sm">{row.userEmail}</p>
                  </div>
                ...
                <div className="bg-gray-100 rounded p-3 space-y-1">
                  <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">MoMo Cashback Details</p>
                  {hasMomo ? (
                    <>
                      <p className="text-sm"><span className="text-gray-400">Name:</span> <span className="text-white font-bold">{row.momoName}</span></p>
                      <p className="text-sm font-mono"><span className="text-gray-400">Number:</span> <span className="text-green-400 font-bold">{row.momoNumber}</span></p>
                    </>
                  ) : (
                    ...
                    <tbody>
                      {row.earnings!.map((e: any) => (
                        <tr key={e.id} className="border-b border-gray-200/50">
                          <td className="py-1.5 pr-3 text-gray-700">{e.refereeName}</td>
                          <td className="py-1.5 pr-3 text-gray-500 uppercase">{e.plan}</td>
            ...
            {users.map(u => (
              <tr key={u.id} className="hover:bg-gray-100">
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded text-xs ${u.subscriptionPlan !== 'none' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'text-gray-400'}`}>
